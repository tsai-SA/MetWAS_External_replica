...
MetS_pheno_covs <- merge(MetS_pheno, covs_mdd, by = id_col)
...
covs_ls <- colnames(covs_mdd)[colnames(covs_mdd) != id_col]
covs_formu = paste0(covs_ls, collapse = " + ")

formula <- as.formula(
    paste0("as.factor(antidep_expo) ~ scale(AD_MetS) + ", paste(covs_formu, collapse = " + "))
)
